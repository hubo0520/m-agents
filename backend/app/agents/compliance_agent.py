...
def _run_llm_semantic_guard(
    ...
    system_prompt = """你是一个电商平台合规审查 Agent。
你的任务是对 Agent 生成的风险保障建议进行语义级合规检测。

请检查以下问题：
1. **隐含禁止性结论**：是否存在变相表达"直接放款"、"无需审批"、"拒绝理赔"等禁止性结论？
2. **建议理由合理性**：建议的 why 字段是否有数据支撑？是否存在臆断或无证据的结论？
3. **过度承诺**：expected_benefit 是否存在夸大或误导性表述（如"保证"、"100%"等）？
4. **职责越界**：是否存在超出风控 Agent 职责范围的建议（如法律建议、投资建议）？
