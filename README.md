...
└─────────────────────────────┼───────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────────┐
│                     FastAPI Backend (V3.1)                           │
│                                                                     │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────────┐  │
│  │  REST API    │  │ JWT Auth +   │  │   Tool Registry          │  │
│  │  (52 routes) │  │ RBAC 中间件  │  │ (幂等 + 审批拦截)         │  │
│  └──────┬───────┘  └──────────────┘  └──────────────────────────┘  │
│         │                                                           │
│  ┌──────▼───────────────────────────────────────────────────────┐  │
...
├── openspec/                          # 变更规范与设计文档
│   └── changes/
│       ├── admin-auth-management/     # 认证系统变更设计
│       └── premium-frontend-redesign/ # 前端视觉重构设计
│
└── v3.md                              # V3 阶段详细 PRD
```
...
- ✅ A3 (诊断) / A5 (建议) / A7 (合规) / A9 (总结) 已接入 LLM
...
系统共 **26 张数据库表**，完整 ER 图和字段说明请查看 👉 [docs/data-model.md](docs/data-model.md)
...
```
V1 (MVP)                       V2 (闭环执行)                   V3 (多Agent生产化)            V3.1 (认证+LLM)
─────────────────►──────────────────────────►──────────────────────────────►──────────────────────────►
• 风险案件分析                  • 融资/理赔/复核任务             • LangGraph 9-Agent 编排       • JWT 认证体系
• 3 Agent 编排                  • 执行闭环                       • 审批门禁 + 守卫系统          • 用户管理 (CRUD)
• 规则引擎 + 指标计算           • 任务状态机                     • Checkpoint 持久化            • 超管初始化引导
• 案件列表 / 详情               • 审批服务                       • 三级降级 (L1/L2/L3)          • 通义千问 LLM 接入
                                                                 • RBAC 5 角色权限              • bcrypt 密码哈希
                                                                 • 评测中心 (离线+抽样)          • Token 自动刷新
                                                                 • Prompt/Schema 版本管理        • 前端 AuthProvider
                                                                 • 灰度开关                      • 认证中间件
                                                                 • 工具注册中心 (幂等+审批)
                                                                 • 46 API 端点
```
