...
export function StatCard({ label, value, icon, trend, color = "text-slate-900", className = "", onClick }: StatCardProps) {
  return (
    <div
      className={`bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs hover:shadow-sm transition-all duration-200 ${onClick ? "cursor-pointer hover:border-slate-300" : ""} ${className}`}
      onClick={onClick}
    >
